...
export function FileTabContent(props: { tab: string }) {
  ...
  const saveFile = async (content: string) => {
    ...
    await file
      .write(p, content)
      .then(() => showToast({ title: language.t("toast.file.saved.title") }))
      .catch((err: unknown) => {
        showToast({
          variant: "error",
      ...
      <Switch>
        ...
        <Match when={state()?.loaded && !isMedia()}>
          <MonacoEditor
            ...
            filePath={path() ?? ""}
            search={search}
            onSave={(val) => void saveFile(val)}
            onSelectionAdd={(range, comment) => {
              const p = path()
              if (!p) return
              prompt.context.add({
                type: "file",
                ...
                preview: previewSelectedLines(contents(), range),
                comment: comment || undefined,
              })
            }}
            addToChatLabel={language.t("editor.addToChat")}
            class="h-full"
          />
        </Match>
        <Match when={state()?.loaded && isMedia()}>
