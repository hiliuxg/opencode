...
export const dict = {
  ...
  "skills.kb.creator.prompt": "使用`skill-creator`技能创建一个**数据分析专属技能（Skill）**\n\n## 技能的核心目标\n该技能旨在赋能大模型，使其能够准确回答用户关于目标数据表的数据分析问题。主要覆盖两个核心场景：\n1. **简单的数据问询**（例如：昨日会员的收入流水是多少？）\n2. **复杂的数据分析**（例如：归因分析的思路和具体数据下钻模板）\n\n## 用户的输入参数\n以下是当前任务需处理的底层数据表信息（请作为 MCP 调用的输入参数）：\n- 计算引擎：{{engine}}\n- 集群名称：{{cluster}}\n- 表名称：{{tables}}\n- 所属业务：{{catalog}}\n- 业务用途：{{purpose}}\n\n## 执行步骤与核心指令\n\n### 第一步：获取表字段信息\n请首先通过 MCP 工具 `get_table_columns` 获取目标表的结构数据。\n- **调用参数**：严格使用上文提供的“计算引擎”、“集群名称”和“表名称”进行查询。\n- **目标**：获取该表的真实字段名称、字段类型以及字段的业务备注信息。\n\n### 第二步：生成标准技能目录结构\n获取到表结构数据后，在使用`skill-creator`初始化技能时，请按照以下精简而专业的技能架构，为该数据表规划文件内容：\n\n```text\n<技能名称>/\n├── SKILL.md\n└── references/\n    ├── schema.md\n    ├── metrics.md\n    ├── simple_query_flow.md\n    └── complex_analysis_flow.md\n```\n\n### 第三步：按原则输出具体文件内容\n在编写这 5 个 Markdown 文件时，请时刻牢记“只补充大模型缺失的上下文”的原则。\n\n#### 1. `SKILL.md`（技能主入口文件）\n- **元数据（Frontmatter）**：包含 YAML 格式的 `name`（技能名称，基于所属业务生成一个简单名字）和 `description`（基于业务用途生成，以及什么时候触发该技能（重要！！！））。\n- **简介与意图路由（Medium freedom）**：简要说明技能用途。设立清晰的意图识别规则，强制要求大模型在响应前，必须先评估用户意图，并在查阅 `simple_query_flow.md`（简单问询）与 `complex_analysis_flow.md`（复杂分析）之间二选一。\n\n#### 2. `references/schema.md`（领域知识：数据表结构）\n- **绝对真实（Low freedom）**：必须根据你第一步获取到的**真实字段数据**来编写，绝对不允许编造任何字段。不用把每个字段全抄，挑出有业务分析价值的核心字段重点说明即可。\n- **补充元数据（Low freedom）**：必须在文档中明确补充该表所属的“集群名称”和“计算引擎”，确保大模型具备完整的执行环境上下文。\n- **查询避坑（Low freedom）**：如果表有分区字段dt，在生成SQL时候，必须携带，而且一定要加上 limit 字段，避免大模型生成全表扫描的SQL。\n\n#### 3. `references/metrics.md`（领域知识：核心指标字典）\n- **强规则约束（Low freedom）**：使用显眼的 Alert 区块（如 `> [!IMPORTANT]`）强调底线纪律。例如：**所有SQL查询必须强制加上 `LIMIT 5` 避免SQL全表扫描**；必须指定实例为特定的集群和计算引擎等。\n- **指标与维度口径**：基于表结构，生成3-5个指标口径，包括指标的计算逻辑、维度口径等，参考下面的模板：\n```text\n## 会员订单指标 (Order Metrics)\n\n**指标描述**：简短描述指标用途，用于什么场景。\n\n### 指标定义\n\n*   **所属表名**: {{tables}}\n\n*   **计算公式**: `SUM(order_pv)` \n\n*   **过滤条件**: `order_effect_type <> '自动续费' AND model = 'svip'`\n\n*   **join条件**: 如果有多表关联，需要说明关联的表名和关联的字段\n```\n\n#### 4. `references/simple_query_flow.md`（简单数据问询流）\n- **SOP 流程（Medium freedom）**：为简单数据问询设计简明的流水线步骤，目的就是正确将用户的自然语言转化成功SQL，然后执行SQL，获取答案\n  1. 理解时间参数。\n  2. 查阅 schema 和 metrics 确定正确的字段口径。\n  3. 生成 SQL 并调用工具执行。\n  4. 严谨的结果核对（强调数据绝对不能错）。\n\n#### 5. `references/complex_analysis_flow.md`（复杂数据分析流）\n- **波动归因与下钻模板（High freedom 结合 Medium freedom）**：该流程核心用于解决**当用户问到某个指标上涨或下跌时，大模型能按固定的模板进行下钻归因**。你需要：\n  - **智能推演模板**：基于获取到的表结构和字段语义，**合理猜测并生成该业务专属的归因和下钻模板**。\n  - **下钻决策树**：梳理出具体的**维度下钻决策树路径**（例如：先看大盘 -> 再看平台 -> 接着看产品线/渠道/分类）。\n  - **算式与步骤**：提供计算维度绝对贡献值的算式规则，以及针对典型下钻路径的单步分析思路和相应的 SQL 代码片段模版。\n- **汇报格式**：提供该模版最终面向用户输出结论的**标准 Markdown 汇报格式**（要求必须包含：整体波动概览、主要排名的因子贡献度表格、以及细拆说明）。\n\n### 其他要求\n1. 所有文件都使用中文输出，除了表、字段等专业名词\n2. 大模型的上下文非常宝贵，所以，在意思表达清晰的前提下，节省的token输出\n",
